...
def slam(depth_file_names, color_file_names, intrinsic, config):
    ...
    return model.extract_pointcloud(), model.voxel_grid, poses
...
if __name__ == '__main__':
    ...
    #config.path_dataset = './pc_stream/real_sense/dataset/realsense/realsense.bag'
    ...
    if not os.path.exists(config.path_npz):
        pc, volume, poses = slam(depth_file_names, color_file_names, intrinsic,
                             config)
    ...
    #mesh = extract_trianglemesh(volume, config, 'output_mesh.ply')
    o3d.t.io.write_point_cloud('output_pc.ply', pc)
